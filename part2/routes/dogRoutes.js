...
router.get('/', async (req, res) => {
  try {
    const [dogs] = await db.execute('SELECT Dogs.dog_id, Dogs.name AS dog_name, Dogs.size, Users.username AS owner_username FROM Dogs JOIN Users ON Dogs.owner_id = Users.user_id');
    res.json(dogs);
  } catch (error) {
